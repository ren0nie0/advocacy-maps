...
class DataPage:
    ...
    def query_page(self, table_header, table_footer, string=""):
        if not string:
    ...
    def get_lobbying_activity(self, wide_query):
        ...
        for query_result in query_results:
            activities_table = self.query_page(header, footer, query_result)[0]
    ...
    def get_client_compensation(self):
        ...
        header = r"".join(columns)
        footer = r'Total salaries received'
        query_results = self.query_page(header, footer)
        for query_result in query_results:
